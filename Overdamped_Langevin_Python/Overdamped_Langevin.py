...
class Langevin:
    def __init__(self, dt, Nt, a, eta0=0.001, T=300, X0=0):
        """
    ...
    def trajectory(self, output=False):
        ...
        for i in range(self.Nt-1):
            Wn = random_gaussian()
            self.Xn[i+1] = self.Xn[i] + np.sqrt(2*self.D0) * Wn * np.sqrt(self.dt)
        if output:
            return self.Xn
    ...
    def MSD(self):
        ...
        """
        self.list_Ntau_MSD = np.array([], dtype=int) #Liste des entiers Ntau tel que tau = Ntau*dt
        for i in range(len(str(self.Nt)) - 1):
            # Prend 10 points par décades
            self.list_Ntau_MSD = np.concatenate(
                (
                    self.list_Ntau_MSD,
        ...
        x = self.Xn
        NumberOfMSDPoint = len(self.list_Ntau_MSD)
        self.MSD = np.zeros(NumberOfMSDPoint)
        for n, i in enumerate(self.list_Ntau_MSD):
            if i == 0:
                self.MSD[n] = 0
                continue
            self.MSD[n] = np.mean((x[i:] - x[0:-i]) ** 2)

        return self.list_Ntau_MSD*self.dt, self.MSD
